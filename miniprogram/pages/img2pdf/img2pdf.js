...
Page({
  ...
  async onStartConvert() {
    ...
    try {
      ...
      wx.redirectTo({
        url: `/pages/result/result?downloadUrl=${encodeURIComponent(result.downloadUrl)}&filename=${encodeURIComponent(result.filename)}&size=${result.size}&convertType=img2pdf&downloadKey=${encodeURIComponent(result.downloadKey || '')}&localPath=${encodeURIComponent(result.localPath || '')}`,
      });
    } catch (err) {
      this.setData({
        status: 'error',
