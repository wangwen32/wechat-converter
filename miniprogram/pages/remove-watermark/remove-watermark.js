...
Page({
  ...
  async onStartConvert() {
    ...
    try {
      ...
      wx.redirectTo({
        url: `/pages/result/result?downloadUrl=${encodeURIComponent(result.downloadUrl)}&filename=${encodeURIComponent(result.filename)}&size=${result.size}&convertType=remove-watermark`,
      });
    } catch (err) {
      this.setData({
        status: 'error',
